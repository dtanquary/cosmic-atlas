...
## Default spiral appearance

The default renders verified road-trip targets with image-inspired models and other source families as spiral illustrations. Settings → Galaxy appearance → Catalog types restores the other measured/proxy families described above; explicitly requested named portraits remain in both modes. Original type metadata, Gaussian profiles, exact positions and adopted radii/sky ellipses are retained; only rendering parameters change. The common exponential profile, assumed thin depth, arm/ring recipes and colors in this mode are visual choices. They must not be presented as a measured morphology distribution. `scripts/prepare_spiral_profile.py` regenerates its small Gaussian profile from the analytic exponential law.
...
Each resolved catalog or nearby galaxy has a stable illustrative palette keyed to its exact public identity. Disk/arm tones span restrained blue-white to ivory; centers remain warmer, with sparse muted pink light samples. [NASA's true-color M81 example](https://apod.nasa.gov/apod/ap970726.html) provides the broad physical motivation for cooler-looking arms and a warmer nucleus. [Hubble's galaxy overview](https://science.nasa.gov/mission/hubble/science/universe-uncovered/hubble-galaxies/) also illustrates blue clusters and pink star-forming regions. These references inform the visual vocabulary, not the palette assigned to any particular catalog object.

The runtime model contract does not carry calibrated visible-light photometry. Tint values and pink-sample fractions are deterministic display choices, not measurements of stellar age, star formation, extinction or redshift. The disk and core endpoints have separately fixed Rec.709-weighted linear RGB luminance (0.69 and 0.88) to keep hue variation from becoming a brightness cue. The remaining tiny integrated-brightness differences come from colored sample composition and display quantization. The Milky Way retains its reference coloring.

Both the smooth light and existing arm/clump color buffer use the palette; no extra geometry or draw pass is added. Source data, profile coefficients, positions, sizes and the geometry random sequence remain unchanged. Change the restrained color endpoints in `src/galaxy-colors.ts`; preserve the identity-based seed and run the color and shared GPU checks.

## Related disk variants

The default spiral appearance mixes five restrained recipes: classic two-arm, finer four-arm, ringed, tightly wound and feathered spirals. All retain the common exponential light profile, exposure 0.55, radial light-sample distribution, disk thickness, point sizes and total point budget of the previous common spiral. No extra bar or bulge component is added. The ring spreads half of the existing samples around a broad annulus without concentrating them radially; the rest retain their spiral paths. The four-arm version divides the existing samples between more arms rather than adding light.

`galaxy-variants.ts` chooses recipes from a separate hash of the exact public identity: 40% classic, 25% multi-arm, 15% ringed, 10% tightly wound and 10% feathered in hash space. The earlier named assignments (Andromeda tight, Triangulum feathered, NGC 3982 multi-arm) remain available to generic-template diagnostics; their displayed models now use the continuous image-inspired portraits below. These proportions are display choices, not measured population fractions. The same hash seeds the illustrative structure, independent of dense row indices, source morphology, palette hash or residency. Positions, adopted sizes and projected orientation still follow the source. Catalog types retains the other original source/proxy renderers, and the Milky Way retains its separate reference recipe.
...
The optional CMB shell is a separate observer-centered background reference. It changes no galaxy model geometry, appearance, fade range, picking identity, measurement or catalog allocation. Its comparison panel yields to either galaxy inspector. Toggling it does not focus or unfocus a model; View cosmic scale explicitly returns to an external observer-centered view. The optional lookback rings likewise change no model geometry, picking identity, measurement or allocation; the inspector's Light travel time row is Planck18 lookback for DESI records, distance ÷ c for the nearby layer, and hidden for uncertain local positions. [Distance convention and scientific limits](cosmic-horizon.md).

## Road trip cloud illustrations

The LMC uses a soft offset stellar bar with uneven outer patches; the SMC uses a fragmented, elongated stellar haze. These visual recipes take broad inspiration from the differing stellar structures described by [Cioni et al. (2000)](https://arxiv.org/abs/astro-ph/0005057), and the idea of faint emission regions from [ESO's LMC N44 observation](https://www.eso.org/public/news/eso1125/). No bar angle, individual nebula, dust distribution or gas density was extracted from those sources. No images are bundled. The cloud models are galaxies containing stars, gas and dust, not standalone gaseous objects or a measured Magellanic Stream.

Positions, distances, scale parameters and projected reference frames are unchanged. LMC orientation and size remain explicitly assumed; SMC retains its cited sky ellipse and assumed half-light scale. The irregular light distribution is not a fit to the reference ellipse or half-light photometry. Its depth, fine structure, colors and dust attenuation are illustrative. Captions and the inspector disclose these limits. Both appearance modes retain these cloud models and the named portraits below; Catalog types restores the remaining source/proxy families.

The earlier feathered Triangulum sample recipe was inspired by the patchy young-population appearance discussed in [Smercina et al. (2023)](https://arxiv.org/abs/2308.11618). It does not fit their resolved stars, recover the measured old-population bar, or change the adopted single disk. The tightly wound and feathered recipes only redistribute existing samples in azimuth, retaining the same radial/depth samples, colors and brightness budget as the common spiral.

## Image-inspired targets

The [stop-by-stop image review](image-portraits.md) records primary images and fallback decisions. Andromeda uses a continuous warm disk with uneven dust arcs, Triangulum a weak-centered patchy blue disk, and NGC 3982 intricate dust and stellar arms. These reuse the home volume integration template with smaller fixed textures, separate recipes and one draw, without changing catalog geometry. M32, M110 and NGC 4026 use the existing smooth elliptical/lenticular profiles. M32/M110 use illustrative exposures of 8/6 for legibility; these change neither profile coefficients nor adopted geometry and are not calibrated photometry. The cloud recipe now follows broad SMASH image features: LMC stellar bar and localized Tarantula-inspired emission, SMC diffuse body/wing and scattered patches. Foreground stars/clusters, image edges and unrelated background emission are excluded. Inspector links compare reference images while keeping adopted shape and independent distance provenance visible.
