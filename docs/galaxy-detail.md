...
Positions, distances, scale parameters and projected reference frames are unchanged. LMC orientation and size remain explicitly assumed; SMC retains its cited sky ellipse and assumed half-light scale. The irregular light distribution is not a fit to the reference ellipse or half-light photometry. Its depth, fine structure, colors and dust attenuation are illustrative. Captions and the inspector disclose these limits. Both appearance modes retain these two new cloud models; Catalog types still restores the other source/proxy families.

The feathered Triangulum illustration is inspired by the patchy young-population appearance discussed in [Smercina et al. (2023)](https://arxiv.org/abs/2308.11618). It does not fit their resolved stars, recover the measured old-population bar, or change the adopted single disk. The tightly wound and feathered recipes only redistribute existing samples in azimuth, retaining the same radial/depth samples, colors and brightness budget as the common spiral.
