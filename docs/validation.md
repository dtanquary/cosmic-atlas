...
All **52 repository tests** pass (91 with `npm test`, which currently also collects the ignored `.cache` clone; ten are new). Strict TypeScript and `npm run build` pass. `prepare_lookback.py` records a maximum comoving→lookback interpolation error of **1.1 × 10⁻³ Gyr** at the 256-row table's widest comoving interval, around z ≈ 1.5 (about 4,500 Mpc), measured deterministically at every row midpoint plus the rings and random redshifts; every ring round-trips within that budget (the 10 Gyr ring errs 1.0 × 10⁻³ Gyr, the 13 and 13.5 Gyr rings under 2 × 10⁻⁴) and the sub-0.44 Mpc branch reproduces distance ÷ c (about 26,500 years at the adopted Galactic-center distance).
