...
All **88 repository tests** pass with `npm test` (**49** excluding the ignored `.cache/**` clone fixture; seven are new). Strict TypeScript and `npm run build` pass. `prepare_lookback.py` records a maximum comoving→lookback interpolation error of **1.1 × 10⁻³ Gyr** at the steep high-redshift end of the 256-row table; every ring round-trips within that budget and the sub-0.44 Mpc branch reproduces distance ÷ c (about 26,500 years at the adopted Galactic-center distance).
