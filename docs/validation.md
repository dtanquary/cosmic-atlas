...
Final acceptance: full and subset GPU reports and the production control/layout report are retained in [validation-results.json](validation-results.json). All sampled shell checksums match before/after context restoration. The public-source/history audit passes. The feature is committed and available in the local production preview; this experimental addition has not been uploaded to the public host.
