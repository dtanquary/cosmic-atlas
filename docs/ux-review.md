...
**Tour controls — 14 September 2026, first milestone verified.** Added short visual cues and a progress control that opens Stops & pace. Chapters and Return to stop land paused; Manual cannot autoplay, and Relaxed extends only dwell. Back restores a verified previous view from bounded session history. The new phone options control retains a 44 px touch target, with a compact portrait panel target of at most 215 px; short landscape hides the cue until details expand. Existing source captions, camera destinations and rendering are unchanged. Final implementation `6a6ebe1` passes 98 tracked tests, 50 new production Chrome/WebKit assertions and full/subset GPU navigation/recovery. The unchanged mobile/invitation controls pass 126 and 30 assertions. Further phase 2 refinements remain pending in the implementation record; physical-phone testing is still pending.

History QA reproduced an important distinction: visit Andromeda, pan, visit Triangulum, then Back. The first implementation restored the selected galaxy but discarded the pan, because public-link restoration deliberately anchors at the verified object center. History now opts into preserving its stored target while retaining the same identity verification and cancellation. Public links keep their existing anchoring contract. The production journey includes the failing pan scenario and Back after opening a shared view; final results are recorded below.
...
Follow-up from inspecting the individual dwarf screenshots: the inherited center-normalized Sérsic profiles left M32/M110 too faint at their adopted scale. Named illustrative exposure is adjusted (8 and 6 respectively), without modifying profile coefficients, size, ellipse or depth. The portrait GPU suite now includes all three smooth models as well as the disks. Full/subset GPU checks and the 54-check production tour pass. Individual production close-ups confirm legible smooth light with no changed radius or ellipse.

## Mobile navigation — 12 September 2026

State: verified on the production package in Chrome and WebKit, plus the shared full-data GPU suites.

The previous 402×874 layout placed a 304×594 inspector over most of the map and kept the desktop icon rail. The new compact galaxy panel is approximately 130 px tall, with name, distance, Focus and Details. Expanded details occupy at most 58% of portrait height (capped at 480 px); the header actions remain fixed while content scrolls. Tours retain Prev/Play/Next with optional captions. A four-action bottom toolbar replaces the rail, and settings use three expandable groups. Initial phone captures confirm reachable controls and substantially more visible sky. Strict TypeScript, 91 tracked tests and 122 final production browser assertions pass. Chrome touch injection verifies orbit, pinch and pan through the public saved-camera flow, preserving the selection. Chrome and WebKit cover 375×812, 402×874, 430×932 and 874×402 layouts, plus reduced-height search and repeated restoration to 1600×1000 desktop. Header actions remain reachable while details scroll; nearby aliases and exact named DESI IDs remain available when expanded. Search cancellation/reopening, clipboard denial, measurement, flight, CMB and tour controls pass. These are browser simulations, not a physical iPhone check. The shared full-data UI, home, share, tour and nearby GPU suites pass, including context recovery. Desktop header, rail, footer, inspector and action bounds match the pre-change capture. Final phone/menu/details/tour/search/landscape captures were visually inspected.

## Tour invitation sharing — 12 September 2026
...
## Ordered milestone 4 — verified sharing and custom trips

Final source `18256b1` passes **124 tests** and the strict full-data package. Its navigation/rendering source `5b27527` passes full 14,140,375-record model validation, full-data tour/UI/home/share/nearby/continuity GPU checks and subset tour/share/nearby checks on Chrome 153 / ANGLE Metal Apple M3 Max. Graphics recovery passes, including the new camera-only selection-clear regression. A camera-only stop previously retained the prior selection; the new regression verifies both its pose and null public identity. A skipped destination's notice was immediately replaced; its bounded summary is now repeated after successful arrival.

The production custom-trip journey passes **83 assertions** on Chrome 153 and WebKit 26.6. It covers save/open/delete, button reordering, saved/current views, plain-text Unicode notes, local/file/link round trips, explicit recipient Start, paused direct stops, missing identity fallback, actual development-subset skips and visible notices, oversized files/links, blocked storage/clipboard, delayed metadata and name-index cancellation, consumed-link reloads and graphics recovery. Desktop, portrait and landscape captures were reviewed. Reproduce with the documented isolated Playwright setup and `node scripts/check-trip-ui.mjs`. `ATLAS_TRIP_ENGINES` can select one engine for diagnosis; final acceptance used both.

Final shared production regressions pass **50 tour/history, 126 mobile, 30 existing invitations, 221 photographs, 493 all-stop and 17 partial-arrival assertions**. Together with the new journey, that is **1,020 browser assertions**. The package is **3,070 files / 1,081.9 MiB**, largest **3.24 MiB**, with unchanged full catalog coverage and bounded photo assets. Release audit passes. Results are in `milestone-results.json`; operational receipts remain ignored. Browser phone layouts/touch gestures run on the Mac GPU. Physical iPhone performance, device interruptions and transfer through real messaging apps remain untested.
