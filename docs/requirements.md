...
- Every accepted galaxy outside the local-distance safeguard supports an automatically streamed close-up model. Provide spiral, barred spiral, elliptical, lenticular and irregular variants. In Catalog types appearance, use recorded visual types where matched; label other choices as approximations. Default to spiral illustrations with dedicated Magellanic Cloud models, with the override clearly disclosed and source classifications preserved. Preserve usable measured size and sky ellipse. Missing shapes use a clearly disclosed 5 kpc comoving half-light radius and unknown orientation. Retain the original NGC 4026 and NGC 3982 previews.
