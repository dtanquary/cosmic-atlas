...
try{
 for(const engine of process.env.ATLAS_TEST_ENGINE?[process.env.ATLAS_TEST_ENGINE]:['chrome','webkit']){
  ...
  try{
   ...
   await act(p,'#copy-road-trip-button');const link=await p.evaluate(()=>window.__copiedLink);check('invitationHasOnlyRoute',link===`${new URL(base).origin}${new URL(base).pathname}#tour=road-trip`);check('copyDoesNotStartSenderTour',await p.locator('#tour-panel').isHidden());await p.screenshot({path:`${dir}/${engine}-share.png`});await p.close();
   ...
   await receiver.reload();await ready(receiver);await receiver.waitForTimeout(400);check('reloadDoesNotRestartTour',await receiver.locator('#tour-panel').isHidden());await receiver.close();
