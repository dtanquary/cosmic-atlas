...
For a UI change, start with `app.ts` and `style.css`; Vite updates the page as you edit. For a camera or navigation change, start with `explorer.ts`. For galaxy appearance, use `galaxy-detail.ts` or `milky-way.ts`; the common illustrative disk profile is regenerated by `uv run python scripts/prepare_spiral_profile.py`; keep display effects distinct from measured physical properties.
