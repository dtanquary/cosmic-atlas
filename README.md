...
| [`src/travel.ts`](src/travel.ts), [`src/view-link.ts`](src/view-link.ts) | Camera travel interpolation, and the shareable view-link grammar with its strict decoder |
| [`src/tour.ts`](src/tour.ts), [`src/data/tours.json`](src/data/tours.json) | Tour runner over the animated visits (one dwell timer, pause on input, skip-and-notify), and the two routes whose captions are pinned to sourced data by tests |
| [`scripts/prepare_tours.py`](scripts/prepare_tours.py), [`src/data/tour-sources.json`](src/data/tour-sources.json) | The cited Coma cluster stop: NED position and redshift to Planck18 axes, and the DESI row count around it |
...
| [`src/diagnostics.ts`](src/diagnostics.ts), [`src/ui-diagnostics.ts`](src/ui-diagnostics.ts), [`src/share-diagnostics.ts`](src/share-diagnostics.ts), [`src/tour-diagnostics.ts`](src/tour-diagnostics.ts) | Development-only browser acceptance runners, including the shareable-view, travel and guided-tour checks |
...
| Share or save this view | **Share**; links carry only the camera and the selected identity, saved views stay in this browser and open with a short camera travel |
| Guided tours | **Tours** in the toolbar, then **Start**; Prev / Pause / Next in the panel, drag or scroll pauses, **Escape** or Exit leaves |
...
**Guided zoom-out and road trip** are camera routes over the existing visits, not physical trajectories. The zoom-out climbs from the Sun through the Milky Way, the Local Group and the whole survey to the CMB shell; the road trip visits the Magellanic Clouds, Andromeda and its satellites, Triangulum, two named DESI galaxies, the Coma cluster and the survey. Each caption states the stop's distance and lookback time and says what is measured and what is illustrative; every number is pinned to its source data by tests. Any input pauses a tour, it never changes saved settings (the CMB stop shows the shell and hands your saved choice back afterwards), and catalog stops are skipped with a notice on data subsets. [Stop sources and assumptions](docs/tours.md).
...
| `?sharetest&run=my-share` | Shareable views: link encode/decode on the live camera, links for a named galaxy, a non-resident chunk, Andromeda and the Galactic core, wrong-ID and out-of-range fallbacks, saved views through the real dialog, wheel-cancelled and completed camera travel, and no visible-model eviction during a hop; repeated after graphics recovery; works with subsets |
| `?tourtest&run=my-tour` | Guided tours through the real Tours button, dialog and panel at 10× speed: per-stop framing of the zoom-out against the reset, cosmic-scale and stop data, the CMB shell shown at its stop and the saved choice restored for both states, an immediate pause on a wheel tick with no target drift, Escape exit, the road trip with catalog stops skipped and announced on subsets, the 12-model bound and no visible-model eviction during hops; repeated after graphics recovery; works with subsets |
