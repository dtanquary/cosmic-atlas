...
The CMB shell, lookback rings and (later) survey footprint are observer-centered reference overlays: one draw each when enabled, zero when disabled, never in the GPU ID pass, excluded from counts/IDs/measurements. Lookback times come from the Planck18 table generated by `prepare_lookback.py`; the browser only interpolates. Run `?lookbacktest` with `?cosmictest` for changes to them.
