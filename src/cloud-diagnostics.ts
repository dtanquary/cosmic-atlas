...
import {ResolvedGalaxy} from './galaxy-detail';
import {nearbyDetails} from './nearby-galaxies';

/** Actual GPU checks for the two volumetric cloud illustrations. */
export function probeCloudModels(renderer:THREE.WebGLRenderer,preview?:HTMLElement){
 ...
 try{
  const cases=nearbyDetails().filter(d=>d.cloud).map(data=>{
   ...
   try{
    ...
    return {name:data.name,views,inside:inside.metrics,behind:behind.metrics,dustDifference,repeatError,smallOrbitDifference,reconstructed,memoryBytes:model.memoryBytes,
     passed:views.every(v=>v.litPixels>300&&v.clippedPixels===0&&v.calls===2&&v.pickable)&&inside.metrics.litPixels>100&&inside.metrics.clippedPixels===0&&behind.metrics.litPixels===0&&dustDifference>.1&&original.metrics.mean<clear.metrics.mean&&repeatError===0&&smallOrbitDifference<2&&reconstructed&&model.memoryBytes<=672000};
   }finally{model.dispose()}
  });
  return {cases,passed:cases.length===2&&cases.every(c=>c.passed)&&cases[0].views[0].checksum!==cases[1].views[0].checksum};
