...
export async function runDiagnostics(atlas:Explorer,parent:HTMLElement){
 ...
 if(query.has('tourtest')){const {probeTours}=await import('./tour-diagnostics');report.tours=await probeTours(atlas);show();report.tourContext=await atlas.probeContextRecovery();report.toursAfterRecovery=await probeTours(atlas);show()}
 if(query.has('sharetest')){const {probeShareViews}=await import('./share-diagnostics');report.share=await probeShareViews(atlas);show();report.shareContext=await atlas.probeContextRecovery();report.shareAfterRecovery=await probeShareViews(atlas);show()}
 ...
 if(query.has('footprinttest')){const {probeSurveyFootprint}=await import('./survey-footprint-diagnostics');report.footprint=await probeSurveyFootprint(atlas);show();report.footprintContext=await atlas.probeContextRecovery();report.footprintAfterRecovery=await probeSurveyFootprint(atlas);report.footprintChecksumStable=(report.footprint as {cosmos:{checksum:number}}).cosmos.checksum===(report.footprintAfterRecovery as {cosmos:{checksum:number}}).cosmos.checksum;show()}
