...
export async function probeTours(atlas:Explorer){
  ...
  try{
    ...
    const clusterFramings:{targetErrorMpc:number;distanceError:number;outwardAlignment:number;observerBehind:boolean;passed:boolean}[]=[];
    ...
    }if(/^Partial/.test(s))element('tour-play').click();}return /^Finished/.test(s)},'the road trip to finish',90000);
