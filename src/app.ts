...
import {Tour,tours,type TourState} from './tour';
...
<aside class="inspector tour-panel" id="tour-panel" aria-label="Guided tour" hidden><div class="inspector-header"><div class="panel-top"><div class="eyebrow" id="tour-progress">Guided tour</div><button class="button quiet icon-button" id="tour-exit" aria-label="Exit the tour">${icons.close}</button></div><div class="tour-title" id="tour-title"></div></div><div class="inspector-body"><h2 class="object-name" id="tour-stop-title"></h2><p class="tour-caption" id="tour-caption"></p></div><div class="tour-footer"><div class="panel-actions tour-controls"><button class="button" id="tour-previous" aria-label="Previous stop">Prev</button><button class="button" id="tour-play" aria-pressed="false" aria-label="Play the tour">Play</button><button class="button" id="tour-next" aria-label="Next stop">Next</button></div><p class="fineprint tour-status" id="tour-status" role="status"></p></div></aside>
...
function showSelection(galaxy:Galaxy|null){
 ...
 if(detail){
  ...
  text('profile-kind',atlas.galaxyAppearance==='spiral'?`${galaxyVariant(galaxy.targetId).variant.label} · illustrative · source: ${typeLabel}`:typeLabel);element('profile-appearance').hidden=atlas.galaxyAppearance!=='spiral';
  ...
  text('profile-description',nearby?nearby.shapeNote+' Internal structure, near side, depth and colors remain illustrative.':origin+(measured?'Size and projected ellipse follow the catalog. ':'No usable shape measurement: size is assumed (5 kpc half-light radius), with no measured orientation. ')+ 'Depth, near side, internal structure and colors are illustrative. Sizes are comoving.');
...
function renderTour(state:TourState){
 ...
 text('tour-stop-title',state.stop?.title??'');text('tour-caption',state.stop?.caption??'');
...
async function initialize(){
 try{
  ...
  atlas.onReady=()=>{
   ...
   if(state)void atlas.applyView(state,0);
  };
  ...
  const startTour=async(key:string)=>{
  ...
  element('tour-exit').onclick=()=>tour?.exit();
  ...
  const pauseTour=()=>{if(tourActive()){pausedByInput=true;tour!.pause()}};
  ...
  window.addEventListener('keydown',event=>{if(event.key==='Escape'&&tourActive()&&!atlas.autoFly&&!atlas.flight&&!document.querySelector('dialog[open]'))tour!.exit()},{capture:true,signal:uiLifecycle.signal});
  ...
  element('visit-galaxy-button').hidden=false;enableTours();
