...
import type {ModelDisplay} from './galaxy-detail';
...
<footer class="footer"><div><div class="count-label">Catalog observations</div><div class="count-value" id="galaxy-count">—</div><div class="count-caption" id="dataset-caption">DESI · DATA RELEASE 1</div><div class="local-policy" id="local-policy">Uncertain local positions hidden</div><div class="count-meta"><span><strong id="loaded-count">—</strong> loaded</span><span><strong id="drawn-count">—</strong> submitted</span></div></div><div class="footer-right"><div class="mode-switch" role="group" aria-label="Rendering detail"><button id="adaptive-button" class="active" aria-pressed="true">Adaptive</button><button id="full-button" aria-pressed="false">Full detail</button></div><div class="scale"><div class="legend-row"><select id="units" aria-label="Distance units"><option value="ly">Light-years</option><option value="Mpc">Megaparsecs</option></select><div class="scale-label" id="scale-label">—</div></div><div class="scale-rule" id="scale-rule"></div><div class="scale-note">At focus depth · comoving distance</div></div></div></footer>
...
function showHome(){
 element('home-inspector').hidden=!atlas.homeSelected;if(!atlas.homeSelected)return;
 showHomeFocus();
...
function showSelection(galaxy:Galaxy|null){
 ...
 const uncertain=uncertainLocalDistance(galaxy.distance);
 ...
 text('object-distance-caption',uncertain?'Unreliable redshift-only distance':'Comoving distance from observer');
 ...
 const name=(detail?.data.name!==galaxy.targetId?detail?.data.name:undefined)??galaxySearch?.nameFor(galaxy.id);
 text('object-name',name??galaxy.targetId);text('object-kind',name?`DESI ${galaxy.targetId}`:'DESI target ID');
 if(detail){
  const model=detail.data.model,measured=model?.shapeMeasured!==false;
  text('profile-heading',measured?'Measured global shape':'Illustrative shape');
  text('profile-kind',model?.typeLabel??(detail.data.spiral?'Spiral · NGC 3982':'Lenticular · NGC 4026'));
  ...
  text('profile-description',origin+(measured?'Size and projected ellipse follow the catalog. ':'No usable shape measurement: size is assumed (5 kpc half-light radius), with no measured orientation. ')+ 'Depth, near side, internal structure and colors are illustrative. Sizes are comoving.');
  ...
  text('profile-angle',!measured?'Unknown':Math.hypot(detail.data.shape.e1,detail.data.shape.e2)<.00001?'Unconstrained':`${detail.frame.positionAngle.toFixed(2)}°`);
 }
 ...
 text('object-z',galaxy.z<.0001?galaxy.z.toExponential(3):galaxy.z.toFixed(6));text('object-zerr',Number.isFinite(galaxy.zerr)&&galaxy.zerr>=0?`± ${galaxy.zerr.toExponential(2)}`:'Not available');
}
function showMeasurement(galaxies:Galaxy[],enabled:boolean){
 ...
 text('measurement-hint',galaxies.some(galaxy=>uncertainLocalDistance(galaxy.distance))?'Unreliable separation · includes an uncertain local redshift distance':galaxies.length===2?'Estimated comoving separation · click to start again':galaxies.length===1?'Choose another point in the map':'Click any point in the map');
}
...
async function initialize(){
 try{
  ...
  try{const value=Number(localStorage.getItem('atlas-minimum-opacity'));if(Number.isFinite(value)&&value>=0&&value<=100){element<HTMLInputElement>('minimum-opacity').value=String(value);atlas.setMinimumOpacity(value/100);text('opacity-value',`${value}%`)}}catch{}
  ...
  element('visit-galaxy-button').hidden=false;
