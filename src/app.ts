...
function showSelection(galaxy:Galaxy|null){
 ...
 if(detail){
  ...
  text('profile-kind',detail.data.cloud?`${cloudLabels[detail.data.cloud]} · illustrative · source: ${typeLabel}`:atlas.galaxyAppearance==='spiral'?`${galaxyVariant(galaxy.targetId).variant.label} · illustrative · source: ${typeLabel}`:typeLabel);element('profile-appearance').hidden=!!detail.data.cloud||atlas.galaxyAppearance!=='spiral';
