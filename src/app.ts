...
async function initialize(){
 try{
  ...
  element<HTMLSelectElement>('model-display').onchange=event=>{const value=(event.target as HTMLSelectElement).value as ModelDisplay;atlas.setModelDisplay(value);try{localStorage.setItem('atlas-model-display',value)}catch{}};
  try{const value=localStorage.getItem('atlas-model-display');if(value==='automatic'||value==='focused'||value==='points'){element<HTMLSelectElement>('model-display').value=value;atlas.setModelDisplay(value)}}catch{}
