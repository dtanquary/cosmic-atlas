...
async function initialize(){
 try{
  ...
  registerAgentTools();
  if(import.meta.env.DEV&&(new URLSearchParams(location.search).has('benchmark')||new URLSearchParams(location.search).has('selftest')||new URLSearchParams(location.search).has('detailtest'))){const {runDiagnostics}=await import('./diagnostics');void runDiagnostics(atlas,element('app'))}
