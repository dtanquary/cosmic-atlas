...
import {photosForStop,photosForIdentity,matchedPhotoView,type Photograph} from './photos';
import type {ViewState} from './view-link';
import {setupMobileUI} from './mobile-ui';
import './photo-panel.css';
import './trip-panel.css';
import {TripPanel} from './trip-panel';
import {decodeStopLink,encodeStopLink,decodeTripLink} from './trips';
import type {TourData} from './tour';
...
let photoPanel:PhotoPanel,photoOriginalView:ViewState|null=null;
...
function showSelection(galaxy:Galaxy|null){
 element('inspector-photo').hidden=photosForIdentity(galaxy?.targetId??null).length===0;
...
function renderTour(state:TourState){
 ...
 atlas.setTourLabels(active&&(state.stop?.sourceStop?.id??state.stop?.id)==='andromeda-companions'?['m32','m110']:[]);renderPlace();
 ...
 text('tour-user-note',state.stop?.note?'Trip note: '+state.stop.note:'');element('tour-user-note').hidden=!state.stop?.note;
 ...
 element('tour-retry').hidden=state.status!=='partial';
 text('tour-status',state.status==='preparing'?'Preparing the arrival view…':state.status==='partial'?'Partial data · Retry, Continue, or choose Next':state.status==='travelling'?'Travelling…':state.status==='dwelling'?'Arrived · continuing shortly':state.status==='finished'?'Finished · Exit returns to the map':pausedByInput?'Paused · drag or scroll moved the view':'Paused');
}
...
async function initialize(){
 try{
  ...
  photoPanel=new PhotoPanel(element('app'),{
   match:async(photo,canNavigate)=>{
    if(atlas.selected?.targetId!==photo.identity){notify('Return to this destination before matching its photograph.');return false}
    const state={...atlas.viewState(),target:[...atlas.selected.position] as [number,number,number]},view=matchedPhotoView(photo,state,atlas.camera.fov);
    if(!view)return false;viewHistory.remember(atlas.viewState());return atlas.applyView(view,0,{canNavigate});
   },
   restore:async canNavigate=>{if(photoOriginalView){viewHistory.remember(atlas.viewState());await atlas.applyView(photoOriginalView,0,{preserveTarget:true,canNavigate})}}
  },uiLifecycle.signal);
  ...
  let hashApplied=false,pendingSharedTour=decodeTourLink(location.hash),pendingInvitation=decodeStopLink(location.hash)??decodeTripLink(location.hash),tourStartSerial=0;
  let invalidInvitation=/^#(?:trip|stop)=/.test(location.hash)&&!pendingInvitation;
  atlas.onReady=()=>{
  ...
  const startRoute=async(route:TourData,invitation=false,stop?:string)=>{
   ...
   tour.setPace(invitation?'quick':tourPace);if(stop)tour.jump(stop);else tour.start();
  };
  const startTour=(key:string,invitation=false)=>{const route=tours.find(route=>route.key===key);if(route)return startRoute(route,invitation)};
  const tripPanel=new TripPanel(element('app'),{
   ready:()=>atlasReady&&Boolean(galaxySearch),
   pause:()=>{cancelPendingTour();pausedByInput=false;tour?.pause();atlas.exitFlight();atlas.stopTravel();photoPanel.close(false);document.querySelectorAll<HTMLDialogElement>('dialog[open]:not(#trip-dialog)').forEach(d=>d.close())},
   current:()=>{if(!atlasReady)return null;const hash=shareHash();return hash?{name:(atlas.homeSelected?'Milky Way':atlas.selected?galaxyName(atlas.selected)??'Galaxy':'Saved perspective'),hash}:null},
   views:readViews,start:route=>{viewHistory.remember(atlas.viewState());void startRoute(route,true)}
  },uiLifecycle.signal);
  element('create-trip-button').onclick=element('my-trips-button').onclick=()=>tripPanel.edit();
  const startSharedTour=()=>{
   if(!atlasReady||!galaxySearch)return;
   if(invalidInvitation){invalidInvitation=false;notify('This trip invitation is invalid or uses an unsupported version. The map is still available.')}
   const invitation=pendingInvitation;pendingInvitation=null;
   if(invitation){if('stops' in invitation)tripPanel.showPreview(invitation);else{viewHistory.remember(atlas.viewState());void startRoute(tours.find(t=>t.key===invitation.route)!,true,invitation.stop)}return}
   if(pendingSharedTour){const key=pendingSharedTour;pendingSharedTour=null;void startTour(key,true)}
  };
  window.addEventListener('hashchange',()=>{const hash=location.hash,key=decodeTourLink(hash),invitation=decodeStopLink(hash)??decodeTripLink(hash);if(!key&&!invitation&&!/^#(?:trip|stop)=/.test(hash))return;cancelPendingTour();pendingSharedTour=key;pendingInvitation=invitation;invalidInvitation=!key&&!invitation;if(hashApplied){history.replaceState(null,'',location.pathname+location.search);startSharedTour()}},{signal:uiLifecycle.signal});
  // Input wins while a linked or manually chosen tour waits for catalog names.
  const cancelPendingTour=()=>{pendingSharedTour=null;pendingInvitation=null;tourStartSerial++};
  ...
  element('tour-retry').onclick=()=>{atlas.retry();pausedByInput=false;tour?.returnToStop()};
  ...
  const pauseTour=()=>{photoPanel.cancelMatch();cancelPendingTour();if(tourActive()){pausedByInput=true;tour!.pause()}};
  const openPhoto=(photos:Photograph[],title:string)=>{cancelPendingTour();pausedByInput=false;tour?.pause();atlas.exitFlight();atlas.stopTravel();photoOriginalView=atlas.viewState();photoPanel.show(photos,title)};
  element('tour-photo').onclick=()=>openPhoto(photosForStop(tour?.state.stop??null),tour?.state.stop?.title??'This view');
  element('inspector-photo').onclick=()=>openPhoto(photosForIdentity(atlas.selected?.targetId??null),galaxyName(atlas.selected!)??'This galaxy');
  element('home-photo').onclick=()=>openPhoto(photosForIdentity('core'),'Milky Way');
  const photoNavigation=new Set(['tour-progress','tour-next','tour-previous','tour-play','tour-exit','tour-return','back-view-button','observer-button','home-solar-view','home-galaxy-view','reset-button','cosmic-scale-button','settings-cosmic-scale','visit-galaxy-button','tours-button','focus-button','observed-view-button','start-flight-button','auto-flight-button','fly-button','share-button']);
  document.addEventListener('click',event=>{const target=event.target instanceof Element?event.target:null,button=target?.closest<HTMLButtonElement>('button');if(button?.disabled)return;if(button&&photoNavigation.has(button.id)||target?.closest('#galaxy-results [role="option"],#saved-views [data-open]'))photoPanel.close(false)},{capture:true,signal:uiLifecycle.signal});
  window.addEventListener('keydown',event=>{if(['KeyR','KeyF'].includes(event.code)&&!document.querySelector('dialog[open]')&&!(event.target instanceof Element&&event.target.matches('input,select,textarea')))photoPanel.close(false)},{signal:uiLifecycle.signal});
  atlas.canvas.addEventListener('webglcontextlost',()=>{cancelPendingTour();pausedByInput=false;tour?.pause();photoPanel.close(false)},{signal:uiLifecycle.signal});
  ...
  element('share-button').onclick=()=>{
   ...
   element('copy-stop-button').hidden=!tour?.state.stop||!encodeStopLink(tour.tour.key,tour.state.stop.id);
  ...
  element('copy-stop-button').onclick=()=>{const hash=tour?.state.stop&&encodeStopLink(tour.tour.key,tour.state.stop.id);if(hash)copyShareLink(hash,'Paused stop link copied')};
