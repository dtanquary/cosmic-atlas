...
function renderCosmicContext(){
 ...
 element('cosmic-context').hidden=!enabled||!!atlas.selected||atlas.homeSelected;
...
async function initialize(){
 try{
  ...
  atlas.onReady=()=>{
   element('loading').hidden=true;renderStats(atlas.stats);
  ...
  const applyCosmicHorizon=(enabled:boolean)=>{atlas.setCosmicHorizon(enabled);pressed('cosmic-horizon-button',enabled);element<HTMLInputElement>('show-cosmic-horizon').checked=enabled;renderCosmicContext();try{localStorage.setItem('atlas-cosmic-horizon',String(enabled))}catch{}};
  ...
  try{applyCosmicHorizon(localStorage.getItem('atlas-cosmic-horizon')==='true')}catch{}
